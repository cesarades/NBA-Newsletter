...
for player in bets:
    ...
    bets_results[player] = [
        int(curr_row['PTS'].tolist()[0]),
        int(curr_row['REB'].tolist()[0]),
        int(curr_row['AST'].tolist()[0]),
        int(curr_row['FGM'].tolist()[0]),
        int(curr_row['FGA'].tolist()[0]), (curr_row['MIN'].tolist()[0])
    ]
...
player_stats['MIN'] = pd.to_datetime(player_stats['MIN'],
                                     format='%M:%S').dt.time
...
for city in teams:
    if city not in teammate_stats:
        ...
        df_filtered = player_stats[(player_stats['TEAM_CITY'] == city) & (
            player_stats['MIN'] > pd.to_datetime('00:20:00').time())]
        # Loop over rows and put into the dictionary
        teammate_stats[city] = {}
        for index, row in df_filtered.iterrows():
            teammate_stats[city][row['PLAYER_NAME']] = [
                row['PTS'], row['REB'], row['AST'], row['FGM'], row['FGA']
            ]
...
for player in bets:
    ...
    prompt += player + " had " + str(points) + " points, " + str(
        rebounds) + " rebounds, and " + str(assists) + " assists on " + str(
            shooting_perc) + "% shooting.\n"
...
for player in bets:
    ...
    for mate in teammates:
        if mate == player:
            continue
        prompt += (mate + ": " + str(teammates[mate][0]) + " points, " +
                   str(teammates[mate][1]) + " assists, and " +
                   str(teammates[mate][2]) + " rebounds on " +
                   str(teammates[mate][-2]) + " field goals made of " +
                   str(teammates[mate][-1]) + " field goals attempted.\n\n")

    # Add players averages below this each time
    p_avg = season_averages[player]
    prompt += (player + " averaged " + str(p_avg[0]) + " points, " +
               str(p_avg[1]) + " rebounds, and " + str(p_avg[2]) +
